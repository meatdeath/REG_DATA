...
void CreateNewFile(void) {
    ...
    if( myFile ) {
        ...
        switch( config.regMode ) {
            case I_MODE: myFile.print( F("Unix time,Date,Time,Alarm,Current I(A)\n") ); break;
            case U_MODE: myFile.print( F("Unix time,Date,Time,U-,U+,K-,K+,R-,R+,Hex Dump\n") ); break;
            default:     myFile.print( F("Error! Unknown mode!\n") );
        }
...
void setup() {
    ...
    if( analogRead(UI_SWITCH_PIN) > 0x7F ) {
        ...
        config.vMax = 2000;
        config.vMin = 0;
        context_size = I_CONTENT_SIZE;
        Serial.print(F(" I mode"));
    } else {
        config.regMode = U_MODE;
        config.vMax = 824;
        config.vMin = 0;
        context_size = U_CONTENT_SIZE;
        Serial.print(F(" U mode"));
    }
    Serial.println(F("... "));
    ...
    Serial.print(F("Init Time system... "));
    ...
    // Чтение времени
    if (RTC.now().year() < 2026)
    {
        unix_time = 1777593600; // 2026/05/01 00:00:00
    }
    else
    {
        unix_time = RTC.now().unixtime();
    }
    last_hour = DateTime( unix_time ).hour();
    ...
    Serial.print( (int)Clock.getTemperature(), DEC );
    Serial.println( F("C") );
...
inline void ShowTemperature(void)
{
...
inline void updateTimeTisplay(DateTime now)
{
    if ( time_print_time ) {
        // Время изменилось
...
// Старт/Стоп записи данных в файл
inline void recordingStartStop(uint8_t hour)
{
...
inline void processDataU(DateTime now)
{
    led_on(LED_WR, 2);
...
inline void processDataI(DateTime now)
{
    // Валидация первого байта
    if (content_byte_index == 0)
    {
        uint8_t count = 0;
        while( read_len && (content.buf[0]&0xF0) != 0 ) { // Первый байт должен бать не более 0x0F, все что не подходит - выбрасываем
            read_len--;
            content.buf[0] = content.buf[1];
            count++;
        }
        Serial.print(F("Skipped number of bytes higher than 0x0F: "));
...
void loop() {
    ...
    else 
    {
        ...
        if( sd_rec_enable ) {              // Если разрешена запись на SD карту
            if( last_hour != reg_hour && (reg_hour%5) == 0 ) { // Создаем новый файл каждые 5ч
                Serial.println(F("5h edge reached"));
                CreateNewFile();
            }
            last_hour = reg_hour;
        }
        ...
        if( read_len ) {                                            // Если что-то считали из порта
            ...
            // Таймаут! Переходим в состояние приема первого байта
            if (content_byte_index > 0)
            {
                Serial.print(F("Number of dropped bytes: "));
                Serial.println(content_byte_index);
                content_byte_index = 0;
            }

            // Вывод времени на дислей
            updateTimeTisplay(now);
        
            // Читаем состояние кнопок
            buttons_process( button_change, button_set, button_start );
        
            if ( button_start ) // Нажата кнопка СТАРТ/СТОП
            {
                // Нажата кнопка Пуск/Стоп
                recordingStartStop(reg_hour);
            } 
            else if ( button_change ) // Нажата кнопка ИЗМЕНИТЬ
            {
                Serial.print(F("Show temperature: "));
                ShowTemperature();
                delay(3000);
            } 
            else if ( button_set && !sd_rec_enable ) // Запись выключена и нажата кнопка УСТАНОВИТЬ
            {
                Serial.println(F("Enter time set mode...\n"));
